...
travel_time_dict_step = {}
for OD in travel_time_dict:
    if OD[0] in locations and OD[1] in locations:
        travel_time_dict_step[OD] = travel_time_dict[OD]

def similarityEvaluationForParameterSet_step1(input_parameters):
    ...
    links = list(travel_time_dict_step.keys())
...
##############################################################
# SPSA Optimization
# input_parameters = [6,8,1,1.2,1]
input_parameters = [6,8,1,0,0]
bounds = [[0,10],[0,10,],[0,5],[0,10],[0,5]]
# # input_parameters = [6,10,1,1.2,1,0.4957,1]
# # # bounds = [[0,10],[0,100,],[0,5],[0,5],[0,10],[0,20],[0,5],[0,5],[0,10],[0,20],[0,5],[0,5]]
# similarityEvaluationForParameterSet_step1(input_parameters)
...
# res = minimizeSPSA_Liang(similarityEvaluationForParameterSet_step1, bounds=bounds, x0=input_parameters, niter=1000, paired=False)
# res = minimizeSPSA_Customize(similarityEvaluationForParameterSet, bounds=bounds, x0=input_parameters, niter=1000, paired=False)
# print res
##############################################################
# # Brute optimization
# rranges = (slice(0,10,1),slice(0,10,1),slice(0, 5, 1), slice(0, 10, 0.1),slice(0,5,1))
# res = optimize.brute(similarityEvaluationForParameterSet_step1,rranges,full_output=True, finish=None)
# ##############################################################
